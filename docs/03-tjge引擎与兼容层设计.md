...
- **资源**：统一 `.bin` 归档（见 `01-bin资源格式.md`），按索引随机定位。
...
4. **静态闸门的局限**：`tsc`/token 骨架同一性/字面量平价/双射检查全绿**不等于**功能已验证——参见 `06-复盘-语义化改名静默崩溃.md`：语义化改名曾在静态闸门全绿下静默崩溃，靠人工实测才暴露。故"回归通过即收敛"需以人工实测为准。

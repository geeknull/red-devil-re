...
### ⭐⭐ ultracode 续（2026-07-18 收官）——关卡3 **完整通关**，结算/通关屏 19→16 **首次与原版逐字节对拍**

上节卡在 boss。**本轮打穿了它，并把 GoalCutscene(19)+MissionComplete(16) 的结算/通关渲染钉进门禁**——
这是此前**任何 scenario/存档注入/模糊器都到不了**的路径（随机输入打不穿 boss，存档注入只送到关卡开头不送到终点）。

**产出（`pnpm verify` 6→7 全绿）**：
- 夹具 `packages/jvm-oracle/regress/game1-beat-level3.txt`（900 帧，`save=3,3,1` 头）+
  `coverage/game1-beat-level3.cover`。**oracle 与 port 逐 op 对拍 41518 条绘制调用逐字节一致**，
  statesSeen 含 **19、16**。
- **变异测试证夹具能抓缺陷且覆盖差分够不到的路径**：把 `BossActor.onProjectileHit` 导弹伤害 `--health` 改成 `-=2`
  → 本夹具**红**（首个失配 F561，正是 boss 战内）、而 `game1-level` 夹具**仍绿**（它根本走不到 boss）→
  **证明本夹具覆盖了既有夹具够不到的路径**（闸门 × 覆盖，缺一不可）。还原→复绿。

**boss 机制补全（上节钉死的基础上，本轮实测新增/修正）**：
- **武器0（导弹）换弹免费＝无限弹**：`PlayerActor.reloadFromReserve` `case 0: magazineAmmo=10`（无备弹消耗）→
  key51 换弹动画结束即补满 10。∴ boss 战**弹药不是约束**（上节曾误以为要够 20 发/须捡 col65 弹药，作废）。
- **boss 每波结算后自动左冲刺**（不止「被碰才冲」）：`updateAtvVehicleBoss` 一波血耗尽 `health=5; phase=2`（:242）
  **不置 phaseTriggered** → 该 phase2→3 冲刺期 `!phaseTriggered && intersectsActor→takeDamage(3)` 仍生效 →
  **冲刺扫过整条竞技场地板，站/蹲都被撞**。∴ 每波（9/8/7 死后共 3 次）必须**腾空躲**。
- **躲法＝leapleft**：boss 进 phase2 蓄力(subTimer≥6)时**先面左(按左)再起跳(UP→leap 64)**，腾空(row<14)让
  左冲的 boss 从脚下穿过；面右起跳会跳进冲刺路径（撞死）——方向是关键。
- **boss 导弹**（row12 平飞左）**蹲伏(DOWN)即躲**（腰以上判定，蹲下穿头而过）；但 DOWN 有**前冲翻滚**副作用
  （`handleDownPress` :1141 `actionId===5||lastInputDir===8&&ihc∈[1,3)`），连续蹲(ihc 恒 0)不翻滚，蹲firing 交替处易触发→撞 boss。
- **打法总纲**：竞技场入口 col~72、hp9；静止窗(boss.targetVelX===0，未碰 boss 时恒静止)开火→撞静止 boss 各 -1；
  20 击穿 4 个有血波→相机解锁(scriptFlagL=false)→走 col82 爬梯(UP)→col85 RescueNpc 着地→GoalCutscene(19)→(自动)MissionComplete(16)。
  末态 hp9、耗时 1:05（时钟已虚拟化，两侧同值）。

**方法（可复现，scratch 已删）**：`packages/behavior-net` 下写**闭环反应式控制器**——真物理 sim 逐帧读内部
（boss phase/tvx/col、玩家 col/hp/ammo/inputHoldCount、来袭导弹）、按规则决策单键、**录键位成脚本**。
参数（camp/backoff/蹲伏阈值/冲刺躲法/触发相位）用**单进程网格搜索**（972 组 37s）扫出通关组合。
**关键教训（又一次「内部声称≠可复现」）**：控制器内部 `reached19=true`，但录出的脚本**独立重放**（新进程 boss-sim）
却 died——因录制**漏了 nav 宏的 keyframe**（只录了 ENTRY+控制器输入）。**独立重放当场揪出**，补录 nav 后才真绿。
→ 印证 verification-epistemics：**「我写了个能通关的控制器」≠「录出的脚本能通关」，必须独立重放/oracle 对拍裁决**。

### 通关脚本录制器 `packages/behavior-net/recorder/`（把夹具变成可复现制品）

上面那条教训催生了这个工具：`regress/*.txt` 通关夹具此前是**不可复现的 blob**（唯一出处是一次性 scratch 控制器）——
与本仓「信任根/可复核」精神抵触。录制器把「策略→脚本」变成**可从源码重生**的一等制品：
- `record.ts` 真物理 headless sim + 逐帧 policy 决策 + 录键位；`policies.ts` 场景/策略注册表；`cli.ts` 录制→自检→写夹具。
- **三层验证**：① 录制阶段自检必达状态；② **独立重放自检**（`replay-states.ts` **子进程+真 behavior-net 适配器**，
  刻意不复用录制器自写 sim→最大独立性，专抓「漏帧/不可复现」）；③ oracle:diff（绝对闸门）。
- game1-beat-level3 已迁入，**逐字节复现原提交**（非注释内容相同），头注释指向重生命令。
- ⚠️ **game2 不做静态重置**：实测 `resetStatics(2)` 清 `LevelScene.cutsceneState` 会破坏开场跳伞过场初始化
  （玩家卡 reserved=4）；game2 与真 game2-adapter 一样依赖新进程原生初始化。

### game2 LevelClear(16) recon（下一单元，已 recon 未打）

并行 fan-out 摸清（**结论：substantial 专项，别当快活**）：LevelClear(16)=`GameCanvas.showResult(true)`，
7 关任一 goal 过场都能触发。**存档跳关约束（实测扫 save[3]）：save[3]=1..6→关卡1..6(Continue)；关卡0 只能 New Game**。
各关 goal：**L0 walk-to-trigger**（净 goal x~1460，但路上抓钩机关 x~470 + 战斗波 x~1286 两道历史未解闸）/
L1 scripted / L2·3·4 walk-to-trigger（未验证闸）/ L5·6 boss。关卡0 机制（键映射/跳伞入关/抓钩=snapToLedge tile==4/
战斗波=相机锁竞技场打光刷怪/**关内按 22 会重开简报**）全部钉死于记忆 [[jvm-oracle]]。
**现状：录制器 game2 init 已通（跳伞过场推进正常），剩 briefing 交接→抓钩→战斗波→goal 四段须逐一打通。**

### ⭐ 本 session（2026-07-18/19）：game2 LevelClear **证难** + **LevelFailed(18) 首次对拍达成** + 单进程重置

按上节起手攻 LevelClear(16)。**结论：LevelClear 经充分探针+真物理搜索证为硬骨头**（与两轮前会话一致）；
**改攻同属 game2 未对拍覆盖缺口的 LevelFailed(18) 并达成**（`pnpm verify` 7→9）。全程四层验证，无一处凑绿灯。

**① 必做第一步：验证录制器 game2 自写 sim（捞出并修好时钟洞，merge 5186250）**
逐帧对拍「录制器 setupSim(2) 自写 sim」vs「真 behavior-net game2-adapter」（game2-level 场景 300 帧）：
gameplay 字段（uiState/subState/sceneSubState/player posX·posY·velX·frameIndex·health·reserved/camera）
**逐帧逐字节全同**——sim 逻辑本身忠实；唯 `levelStartTime`（`GameCanvas:435 = Date.now()`）分歧：133 帧起
adapter=10640（=133×80ms 虚拟时钟）vs recsim=真 epoch 毫秒（**record() 从不装虚拟时钟，跑真墙钟**）→
167/300 帧分歧、不可复现。该字段驱动 `elapsedSeconds`（:673）→ **结算屏耗时显示**，正是本单元要对拍的界面。
修法＝record() 与 driver/replay/oracle 对齐：installClock(0) + 每帧 paint **之后** advance（game1=100/game2=80ms）。
修后同对拍 0/300 帧分歧。**这是典型「参照物错」**：用一个跟真适配器/oracle 不一致的 sim 驱动策略搜索。

**② 全 7 关触发器地图 + goal 机制钉死（字节码/实测）**
每关 2 个 type-2 触发器：GOAL#0 在出生点（subX=0，开场过场）+ end-goal（较大 b18）。`fireTriggerBossScript`
（`LevelScene:1288`）解码 byte-18 → `cutsceneState[0]=b18/10, [1]=b18%10` → `runCutsceneSceneN` → `showResult`。
**goal 触发条件 = 玩家在 goal AABB 内 + 着地（reserved&1）**：触发→BossScript 相位→`PlayerActor.update` BossScript
分支（:296-298）grounded 时置 `dead=true`（`dead` 是「boss脚本定身」误名，非真死）→BossScript→Cutscene→showResult。
- **LevelClear(16)=showResult(true)；LevelFailed(18)=showResult(false)**（`GameCanvas.showResult` :670-682，**共用**
  `elapsedSeconds=(Date.now()-levelStartTime)/1000` 计时）。各关 end-goal：L1 x119-329(与 wave 同 AABB)/L2 x840-929(高)/
  **L3 x24-184,y200-311(出生已在 x 段内、须下探)**/L4 x598-745(高,2 wave)/L5 x938-1072(boss 关,reserved&1 守卫)。
- Continue 跳关：`GameCanvas:368 levelIndex=saveRecord[3]`（save[3]=N→关卡 N，1-6 走 LevelIntroLoading 跳过跳伞过场）。

**③ 🔴 LevelClear(16) 证难（决定性，非「没做到」是「很难」）**
真物理 best-first 搜索（用已验证 sim + 单进程重置，见 ⑤）在 L3 上被两道墙夹死：
- **迷宫启发式陷阱（game1 同款教训）**：L3 goal 在 col13（出生左下方），manhattan 启发式指向**左**，但唯一下探口
  在**最右 col112 的地板缺口**（上层地板 row20-23 全宽实心 cols0-111）→ 启发式把搜索拉向死胡同。
- **col52 致命战斗要冲**：走右到 col52 被多名步兵(t1/t3)+炮台(t7)交叉火力 + 敌弹(t10)覆盖，实测 hp10→0
  **约 60 帧致死**（搜索按死亡剪枝→过不去）。过了它还有 far-right 下探迷宫须绕回左下 goal（路径极长）。
  → 与两轮前会话「game2 关卡是 combat+maze gauntlet」结论一致。**如实标注：本 session 未打通 LevelClear。**

**④ ✅ 改攻 LevelFailed(18) 并达成（merge d479065，`regress/game2-fail-level2.txt`）**
LevelFailed(18) 是**同属 game2 未对拍覆盖缺口**（模糊器 150+33 轮杀不死玩家；game2-level 从不死），且与
LevelClear 共用 showResult→elapsedSeconds 时钟渲染——**正好检验本 session 修的时钟洞**。
- 脚本＝Continue 跳关卡2 → 过开场简报 → 走右撞敌交叉火力致死 → LevelFailed(18)。**oracle 28407 op 逐字节一致**。
- 结算屏对拍：`任务三失败`／`击毙敌人:0`／`继续次数:1`／**`所用时间: 00:20`**（时钟已虚拟化两侧同值）／`继续`／`菜单`。
- **变异测试**：`paintLevelFailed` 计时渲染 `drawString(timeStr,140,116)`→`140,117` → 本夹具**红**、`game2-level`
  夹具**仍绿**（结构上到不了 LevelFailed）→ 闸门 × 覆盖，缺一不可。
- **🔴 关键约束：只能选开场走 `enterBriefing` 的关（L2），不能选走 `MissionDialog` 的关（L3/4/5）**——后者
  `renderDialogBar→drawWrappedLines` 按宽度自动换行，撞 shim 字体度量盲区（`substringWidth` 编造 len*6 vs 真 12/13px
  → 换行断点不同→其后整段推歪→结构性不可对拍）。**实测 L3 LevelFailed 首个失配正是该盲区**（对话文字换行分歧，
  非真缺陷），故避之。L2 开场＝enterBriefing（过场 sub1→MissionBrief→Normal，无 sub4 对话），与 game0 同为干净简报路径。

**⑤ ✅ game2 单进程静态重置（兑现旧 TODO，merge 4664f6a，`reset-game2.verify.ts`，verify 8→9）**
`resetGame2Statics()` 把有状态静态量重置回**类静态初始化块原值**，使单进程内第 2,3… 次 new GameMIDlet 与
全新进程**逐字节一致**（③ 的搜索靠它才能单进程跑）。**踩死的坑**：`ammoReserve` 静态块置 [99,6,3]、
`throwCooldownQueue` 置 4×Int32Array(3)，多清少清都偏离 fresh。制度化＝单进程 3 跑证「不重置→污染、重置→复现 fresh」。

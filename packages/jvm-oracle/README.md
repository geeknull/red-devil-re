...
- 🔴 ③ **两侧不共享时钟**（2026-07-16 由差分模糊器发现，**推翻了本文原先「无需虚拟化时钟」的断言**）：
  ...
  - **影响面已用字节码收紧到「一条 op」**：`D:J` 全部 7 处读写都在 `paint()` 内，两处读都直接进
    mm:ss 格式化再 drawString，**不参与玩法/计分/存档**。实测印证：整轮 130932 op **只差这 1 条**。
    故 `fuzz.ts` 只**逐 op** 豁免它（同帧+同坐标+时间戳前文本全同+仅 mm:ss 不同），其余照常判定。
  - 🔧 **可根治（未做）**：给 oracle 打 javaagent，把 `tjge/*` 里的
    `invokestatic System.currentTimeMillis` 改写到每帧 +W(100/80ms) 的虚拟时钟。
    W 独立取自原版字节码 → **不是抄 port**，不构成「用它自己验它自己」。
